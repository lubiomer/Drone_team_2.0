...
describe('Create Cart', () => {
    ...
    it('should return status code 400 when request body is null', async () => {
        try {
            await request.post(`${process.env.SERVER_URL}/api/carts/create`, {
                json: true,
                headers: {
    ...
    it('should create document when all validation passes', async () => {
        ...
        const cart = await request.post(`${process.env.SERVER_URL}/api/carts/create`, {
            body: sampleCart,
            json: true,
            headers: {
                'Authorization': `Bearer ${accessToken}`,
    ...
    after(async () => {
        ...
        const collectionCart = await getMongodbCollection('carts');
        await collectionCart.deleteOne({ _id: sampleCart._id });
    });
});